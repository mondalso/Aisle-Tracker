...
public class Register_controller {
	@FXML
	private TextField usr_reg;
	@FXML
	private TextField pwd_reg;
	@FXML
	private Label txt_box;
	
	private String path=((Paths.get("login.json")).toAbsolutePath()).toString();
	
	@FXML
	public void register() throws IOException, ParseException{
		String usr=usr_reg.getText(),pwd=pwd_reg.getText();
		JSONParser jsonParser=new JSONParser();
		Object jsonObject=jsonParser.parse(new FileReader(path));
		JSONObject json=(JSONObject)jsonObject;
		if(json.containsKey(usr)) {
			txt_box.setText("Username exists, please re-login");
		    return;}
		else
			json_add(usr,pwd);
		txt_box.setText("Account created!");		
	}
